...
| `eigenpal workflow dataset review-request item [options] <automation-id> <review-id> <item-id>` | Approve, remove, reopen, comment, edit, or record a field- or file-decision on one review item. Upload corrected expected-file bytes with edit-file. |
