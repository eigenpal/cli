...
export async function fetchEvalResults(
  ...
  // Workflow-agnostic endpoint — the server resolves the owning workflow from
  // the batch id (eval_results stores both columns). Two batch ids uniquely
  // identify the experiments; the caller doesn't have to re-type a workflow.
  const raw = (await client.get(`/api/v1/eval-results/export`, {
    format: 'json',
    batchId,
  })) as EvalResultsExportPayload;
  if (!raw || !Array.isArray(raw.results)) {
    throw new Error(
      `Unexpected eval-results response for batch ${batchId}: missing 'results' array`
    );
  }
  return raw;
}
