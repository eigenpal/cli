 ...
 * in src/cli.ts. Walks the program graph, emits one whole-CLI surface file
 * plus one markdown file per top-level command. The package-level README at
...
function renderFullSurfaceFile(): string {
  const lines: string[] = [];
  lines.push('# eigenpal CLI surface');
  lines.push('');
  lines.push(
