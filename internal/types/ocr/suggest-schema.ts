...
import { getDefaultOcrLlmModelId, OcrLlmReasoningEffortSchema } from './llm-models';
import { LlmOptionsRequestSchema } from './model-options';
...
export function resolveSuggestSchemaLlmModel(requested?: string | null): string {
  const trimmed = requested?.trim();
  return trimmed && trimmed.length > 0 ? trimmed : getDefaultOcrLlmModelId();
}
