 ...
 * the entire `function scoreScript(expected, actual): number { … }`
 * declaration as a string. Validation, body extraction, and runtime checks
 * all live in the shared `compileTypedScript` pipeline
 * (`packages/types/src/typed-script`) — this module only owns the canonical
 * default function text used by the dashboard's "reset" action and the
 * server-side schema's seed.
 */
