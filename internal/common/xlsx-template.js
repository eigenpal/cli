 ...
 * Based on xlsx-template by Martin Aspeli (https://github.com/optilude/xlsx-template)
 * MIT License
 */
...
var path = require('path'),
  imageSize = require('@fumari/image-size').imageSize,
  fs = require('fs'),
...
// Keep aligned with JSON_TO_XLSX_DEFAULT_LIMITS.maxRowsPerSheet.
var MAX_TABLE_EXPANSION_ROWS = 100000;
// Keep aligned with JSON_TO_XLSX_DEFAULT_LIMITS.maxColumns.
var MAX_ARRAY_EXPANSION_COLUMNS = 256;
...
class Workbook {
  ...
  copySheet(sheetName, copyName, binary = true) {
    ...
    if (binary === false && !process.env.JEST_WORKER_ID) {
      console.warn(
        'Warning: copySheet() called with binary=false. UTF-8 characters (éàèùï, etc.) in sheet content, headers, footers, comments, and other XML content may be corrupted. Use binary=true (default) to preserve all characters correctly.'
      );
    }
    ...
    sheetContentType.attrib.ContentType =
        ...
        if (sourceFile) {
          ...
          if (
            ...
            binaryContent = binaryContent.replace(
              /(<author>tc=\{)[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}(\}<\/author>)/gi,
              '$1' + uuidWithoutBraces + '$2'
            );
            binaryContent = binaryContent.replace(
              /(xr:uid="\{)[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}(\}\")/gi,
              '$1' + uuidWithoutBraces + '$2'
            );

            var commentsContentType = etree.SubElement(self.contentTypes, 'Override');
            commentsContentType.attrib.PartName = '/' + newFilePath;
            commentsContentType.attrib.ContentType =
            ...
            var uuidWithoutBraces = newCommentUuid.replace(/[{}]/g, '');
            binaryContent = binaryContent.replace(
              /(\sid="\{)[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}(\}")/gi,
              '$1' + uuidWithoutBraces + '$2'
            );
            ...
            threadedCommentContentType.attrib.ContentType =
  ...
  _rebuild() {
    ...
    self.archive.file(
      self.prefix + '/' + '_rels' + '/' + path.basename(self.workbookPath) + '.rels',
  ...
  loadTemplate(data) {
    ...
    self.workbookRels = etree
      .parse(
        self.archive
          .file(self.prefix + '/' + '_rels' + '/' + path.basename(workbookPath) + '.rels')
          .asText()
      )
      .getroot();
    self.sheets = self.loadSheets(self.prefix, self.workbook, self.workbookRels);
    ...
    self.sharedStringsPath =
      self.prefix +
      '/' +
      self.workbookRels.find("Relationship[@Type='" + SHARED_STRINGS_RELATIONSHIP + "']").attrib
        .Target;
    self.sharedStrings = [];
    etree
    ...
    sheetData.findall('row').forEach(function (row) {
      ...
      row.findall('c').forEach(function (cell) {
        ...
        if (cell.attrib.t === 's') {
          ...
          self.extractPlaceholders(string).forEach(function (placeholder) {
            ...
            if (placeholder.type === 'table' && !placeholder.key) {
              throw new Error(
                'Invalid XLSX table placeholder "' +
            ...
            if (placeholder.full && placeholder.type === 'table' && substitution instanceof Array) {
              ...
              if (newCellsInserted !== 0 || substitution.length) {
                ...
                var firstElement = substitution[0];
                var firstField =
                  firstElement != null && placeholder.key
                    ? firstElement[placeholder.key]
                    : undefined;
                if (firstField instanceof Array) {
                  appendCell = false;
                }
              }
      ...
      if (newTableRows.length > 0) {
        ...
        newTableRows.forEach(function (row) {
          // Copy static (non-placeholder) prototype cells onto each generated
          // row. Formula cells are cloned as-is: <f> text keeps the prototype
          // references (same-reference semantics, not Excel fill-down).
          var occupied = {};
          row.findall('c').forEach(function (existing) {
            occupied[self.splitRef(existing.attrib.r).col] = true;
          });
          cellsOverTable.forEach(function (cellOverTable) {
            var col = self.splitRef(cellOverTable.attrib.r).col;
            if (occupied[col]) return;
  ...
  loadDrawing(sheet, sheetFilename, rels) {
    ...
    drawing.relFilename =
      path.dirname(drawingFilename) + '/_rels/' + path.basename(drawingFilename) + '.rels';
  ...
  initDrawing(sheet, rels) {
    ...
    var drawing = {};
    var drawingFilename =
      'drawing' + self.findMaxFileId(/xl\/drawings\/drawing\d*\.xml/, /drawing(\d*)\.xml/) + '.xml';
  ...
  _moveTwoCellAnchor(drawingElement, fromRow, nbRow) {
    ...
    var _moveImage = function (drawingElement, fromRow, nbRow) {
      var from =
        Number.parseInt(drawingElement.find('xdr:from').find('xdr:row').text, 10) +
  ...
  splitRef(ref) {
    var match = String(ref || '').match(/(?:(.+)!)?(\$)?([A-Z]+)?(\$)?([0-9]+)/);
    return {
  ...
  insertCellValue(cell, substitution) {
    ...
    if (cellValue == null) {
      cellValue = new etree.Element('v');
      cell.append(cellValue);
    }

    // Never promote substitution strings into formulas. Untrusted values
    // beginning with =, +, -, or @ must remain shared-string text cells.
    // Existing template <f> cells are not passed through this function.
    cell.findall('f').forEach(function (formula) {
      cell.remove(formula);
    });
  ...
  substituteArray(cells, cell, substitution) {
    ...
    if (substitution.length > MAX_ARRAY_EXPANSION_COLUMNS) {
      throw new Error(
        'XLSX template expansion exceeds maxColumns limit (' +
          MAX_ARRAY_EXPANSION_COLUMNS +
          '): ' +
          substitution.length +
          ' columns. Reduce the array-valued {field} or split columns.'
      );
    }
  ...
  substituteTable(
    ...
    if (substitution.length > MAX_TABLE_EXPANSION_ROWS) {
      throw new Error(
        'XLSX template expansion exceeds maxRowsPerSheet limit (' +
          MAX_TABLE_EXPANSION_ROWS +
          '): ' +
          substitution.length +
          ' rows. Reduce the {table:...} array or split across sheets.'
      );
    }
  ...
  writeRichData() {
    if (this.richDataIsInit) {
      ...
      if (
        !this.writeRichDataAlreadyExist(this.workbookRels, 'Relationship', 'Target', 'metadata.xml')
      ) {
        _rel = etree.SubElement(this.workbookRels, 'Relationship');
  ...
  substituteImageInCell(cell, substitution) {
    ...
    this.archive.file(
      'xl/media/image' + maxFildId + '.' + fileExtension,
    ...
    const XLRICHVALUEMetaDataTypeIndex = this.metadata
      .find('metadataTypes')
      .findall('metadataType')
  ...
  substituteImage(cell, string, placeholder, substitution, drawing) {
    ...
    // put image to media.
    self.archive.file('xl/media/image' + maxFildId + '.jpg', toArrayBuffer(substitution), {
      binary: true,
      base64: false,
    });
